...
$('#draw_area').bind 'add-planet', (x, y, r1, r2, r3, hashs) ->
...
$ ->
  results = []
  values_num = $('#main').find('tr').get().map((e)-> $(e).find('td.value').length).max().first()
  ...
  console.log(results)
  $('#draw_area').trigger('add-planet', 100, 100, 20, 10, 5, hashs)
